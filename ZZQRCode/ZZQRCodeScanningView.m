...
/** 扫描内容的Y值 */
#define scanContent_Y 169*KWidth_Scale

/** 扫描内容的X值 */
#define scanContent_X (SCREEN_WIDTH - 258*KWidth_Scale) *0.5

@interface ZZQRCodeScanningView ()
...
@implementation ZZQRCodeScanningView

/** 二维码冲击波动画时间 */
static CGFloat const SGQRCodeScanningLineAnimation = 0.05;

/** 扫描动画线(冲击波) 的高度 */
static CGFloat const scanninglineHeight = 3;
/** 扫描内容外部View的alpha值 */
...
#pragma mark - - - 扫描外部View的创建
    ...
    CGFloat promptLabelY = CGRectGetMaxY(scanContent_layer.frame) + 20;
    ...
    //闪电图片
    UIButton *flash_button = [[UIButton alloc] init];
    CGFloat flash_buttonX = 120 *KWidth_Scale;
    CGFloat flash_buttonY = CGRectGetMaxY(promptLabel.frame) + scanContent_X * 0.5;
    CGFloat flash_buttonW = 15;
    CGFloat flash_buttonH = 25;
    flash_button.frame = CGRectMake(flash_buttonX, flash_buttonY, flash_buttonW, flash_buttonH);
    [flash_button setImage:[UIImage imageNamed:@"lightning"] forState:UIControlStateNormal];
    [light_button addTarget:self action:@selector(light_buttonAction:) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:flash_button];
...
#pragma mark - - - 扫描边角imageView的创建
    ...
    // 左下侧的image
    UIImage *left_image_down = [UIImage imageNamed:@"QRCodeRightTop"];
    UIImageView *left_imageView_down = [[UIImageView alloc] init];
    left_imageView_down.transform = CGAffineTransformMakeRotation(M_PI);
    ...
    // 右下侧的image
    UIImage *right_image_down = [UIImage imageNamed:@"QRCodeLeftTop"];
    UIImageView *right_imageView_down = [[UIImageView alloc] init];
    right_imageView_down.transform = CGAffineTransformMakeRotation(M_PI);
...
- (UIImageView *)scanningline {
    if (!_scanningline) {
        ...
        _scanningline.frame = CGRectMake(scanContent_X-10, scanContent_Y, self.frame.size.width - scanContent_X*2 +20, scanninglineHeight);
    }
